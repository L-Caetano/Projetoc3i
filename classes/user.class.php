...
class user{
	...
	function __construct() {
     ...
     function upload_new_user(){
     	  if($result = $this->conn->action('SELECT * FROM `usuario` WHERE `nome`= "'.$this->email.'"')){
            if(mysqli_num_rows($result) > 0){
                echo 'E-mail já existente'; 

            }else{
            if($this->conn->action('INSERT INTO usuario(nome,senha) VALUES ("'.$this->email.'","'.$this->password.'")')){
...
//Função usada apos enviar formulario
      ...
      */
     	if($result = $this->conn->action('SELECT * FROM `usuario` WHERE `nome`= "'.$this->email.'" and `senha`="'.$this->password.'"')){

     		if(mysqli_num_rows($result) > 0){
     			 $_SESSION['login']=true;
           

           if($x = $this->conn->action('SELECT iduser,senha FROM `usuario` WHERE `nome`= "'.$this->email.'"')){
           $row=$x->fetch_array();
